...
class Map
{
  ...
  public:
    ...
    void removeStageWall()
    {
        for (int x = 1; x < width - 1; x++)
        {
            if (map[height / 2][x] == 3) // 벽이라면 삭제
                map[height / 2][x] = 0;  // 빈 공간으로 되돌림
        }
    }
    ...
    bool placeGate(time_t now)
    {
        ...
        if (wall_positions.size() < 2)
            return false; // 게이트 생성에 필요한 벽이 부족한 경우
        // random_shuffle() 은 C++17 이상에서 작동하지 않음
        random_device rd;
        mt19937 g(rd()); // 랜덤 엔진
        shuffle(wall_positions.begin(), wall_positions.end(),
                g); // random_shuffle() 과 동일한 동작을 하는 std::shuffle()
    ...
    void setGatePassedBySnake(bool status)
    {
        currentGate.passed_by_snake = status;
    } // 뱀 통과 플래그 설정
    bool getGatePassedBySnake() const
    {
        return currentGate.passed_by_snake;
    } // 뱀 통과 플래그 가져오기
    int getHeight() const
    {
        return height;
...
int main()
{
    ...
    Map gameMap(21 + (stage - 1), 21 + (stage - 1));
    Snake snake(gameMap.getHeight() / 2, gameMap.getWidth() / 2, 5);
    ...
    while (true)
    {
        static time_t wallStartTime = time(NULL);
        static bool wallSpawned = false;
        ...
        if (!wallSpawned && now - wallStartTime >= 15)
        { // 특정 시간마다 벽 출현
            gameMap.spawnStageWall();
            wallSpawned = true;
        }

        if (wallSpawned && now - wallStartTime >= 30)
        { // 벽 등장 시간의 2배가 될 때 벽 제거
            gameMap.removeStageWall();
            wallSpawned = false;
            wallStartTime = now; // 리셋
        }
        ...
        if (ch != ERR)
        {
            ...
            if (ch == KEY_UP)
                ...
                newDir = 3;
            // 반대 방향 입력 시 게임 종료
            if (newDir != -1 && !snake.changeDirection(newDir))
                break;
        }
        ...
        if (gameMap.isGate(next.first, next.second))
        {
            ...
            // 뱀이 어느 게이트로 진입했는지 확인
            if (next.first == currentGateInfo.pos1.first &&
                next.second == currentGateInfo.pos1.second)
            {
            ...
            // 가장자리 벽 게이트 (고정 진출)
            if (exitGatePos.first == 0)
            {                // 상단 벽
                exitDir = 2; // DOWN
            }
            else if (exitGatePos.first == gameMap.getHeight() - 1)
            {                // 하단 벽
                exitDir = 0; // UP
            }
            else if (exitGatePos.second == 0)
            {                // 좌측 벽
                exitDir = 1; // RIGHT
            }
            else if (exitGatePos.second == gameMap.getWidth() - 1)
            {                // 우측 벽
                exitDir = 3; // LEFT
            }
            else
            { // 맵 가운데 벽 게이트
                ...
                int possibleDirs[4] = {currentSnakeDir, (currentSnakeDir + 1) % 4,
                                       (currentSnakeDir + 3) % 4, (currentSnakeDir + 2) % 4};
                ...
                for (int dir_candidate : possibleDirs)
                {
                    ...
                    // 다음 진출 위치 계산
                    if (dir_candidate == 0)
                        potentialExitPos.first--; // UP
                    else if (dir_candidate == 1)
                        potentialExitPos.second++; // RIGHT
                    else if (dir_candidate == 2)
                        potentialExitPos.first++; // DOWN
                    else if (dir_candidate == 3)
                        potentialExitPos.second--; // LEFT

                    // 다음 위치가 벽이 아니고, 뱀 몸통이 아니며 맵 경계를 벗어나지 않는 경우
                    if (potentialExitPos.first >= 0 &&
                        potentialExitPos.first < gameMap.getHeight() &&
                        potentialExitPos.second >= 0 &&
                        potentialExitPos.second < gameMap.getWidth() &&
        ...
        // 벽 충돌 시 게임 종료
                 ...
                 gameMap.getCell(next.first, next.second) == 3)
            break;

        snake.move();
        // 이하 스테이지 통과 코드
        if (snake.getLength() >= 6 && growthCount >= 4 && poisonCount >= 2 && gateCount >= 1)
        {
            stage++;
            if (stage > 5)
                break;
            gameMap.~Map();
            new (&gameMap) Map(21 + (stage - 1), 21 + (stage - 1));
            snake.~Snake();
            new (&snake) Snake(gameMap.getHeight() / 2, gameMap.getWidth() / 2, 5);
            growthCount = poisonCount = gateCount = 0;
            items.clear();
            wallSpawned = false;
            wallStartTime = time(NULL);
            lastGateTime = time(NULL);
        }
        ...
        if (cell >= 5 && cell <= 9)
        {
            ...
            // 공통 처리 부분 if-else 밖으로
            gameMap.setCell(next.first, next.second, 0);
            items.erase(remove_if(items.begin(), items.end(), [&](Item& it)
        ...
        while (items.size() < 3)
        {
            int itemTypes[] = {5, 6, 8, 9};
            int type = itemTypes[rand() % 4];
            placeItem(type);
        }
        double time_since_last_event = difftime(now, lastGateEventTime);

        if (!gameMap.getGate().active)
        { // 게이트가 현재 비활성화 상태일 때
            if (time_since_last_event >= GATE_REGEN_DELAY_SEC)
            { // 사라진 후 5초 뒤에 다시 생성
        ...
        // 이하 상태를 나타내는 GUI
        int info_y = 1;
        int info_x = gameMap.getWidth() + 3;

        mvprintw(info_y++, info_x, "=== STATUS ===");
        mvprintw(info_y++, info_x, "Stage     : %d", stage);
        mvprintw(info_y++, info_x, "Length    : %d", snake.getLength());
        mvprintw(info_y++, info_x, "Growth    : %d", growthCount);
        mvprintw(info_y++, info_x, "Poison    : %d", poisonCount);
        mvprintw(info_y++, info_x, "Gate Pass : %d", gateCount);
        mvprintw(info_y++, info_x, "Combo     : %d", combo);
        ...
        int delay = max(200, 1000 - (stage - 1) * 200);
        napms(delay);
    }
